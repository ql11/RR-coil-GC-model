...
%for i = 1:M_size % 源元素，矩阵行
    ...
    if ten_count==10
        ...
        syms x
        last_time = seconds(double(symsum(toc.*(x/i),x,i,M_size)));
        last_time.Format = 'hh:mm:ss';
        fprintf('循环时间%.2fs,循环数%d/%d,进度%.2f%%,预计仍需%s计算完成\n',toc,i,M_size,schedule,char(last_time));
        ten_count = 0;
    else
        fprintf('循环时间%.2fs,循环数%d/%d\n',toc,i,M_size);
